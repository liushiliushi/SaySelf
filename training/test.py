...
def main():
    ...
    # Remove the manual distributed initialization since DeepSpeed will handle it
    # if dist.is_available() and not dist.is_initialized():
    #     dist.init_process_group(backend='nccl')
        
    # Initialize the accelerator with DeepSpeed settings
    ...
    accelerator = Accelerator(
        ...
        **accelerator_log_kwargs,
        ...
        deepspeed_plugin=None,  # Let DeepSpeed config file handle the settings
        split_batches=True,     # This helps with DeepSpeed integration
    )
    ...
    # Modify the final save logic
    if args.output_dir is not None:
        try:
            # For DeepSpeed ZeRO-3, we need special handling
            if accelerator.distributed_type == "DEEPSPEED" and accelerator.state.deepspeed_plugin.zero_stage == 3:
                # Save using DeepSpeed engine's save_checkpoint
                success = accelerator.save_state(args.output_dir)
                if success and accelerator.is_main_process:
                    tokenizer.save_pretrained(args.output_dir)
            else:
                # Regular save for other cases
                accelerator.wait_for_everyone()
                if accelerator.is_main_process:
                    tokenizer.save_pretrained(args.output_dir)
                save_with_accelerate(accelerator, model, tokenizer, args.output_dir, args)
        except Exception as e:
            logger.warning(f"Error during final save: {str(e)}")
